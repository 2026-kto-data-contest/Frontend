  ...
  return (
    <HeaderContainer>
      <LogoButton type="button" aria-label="홈으로 이동" onClick={() => navigate("/")}>
        ...
        >
          {/* Figma가 내보낸 원본에는 이 도형에 드롭섀도우·이너섀도우 SVG filter가 걸려
              있었는데, 로고가 24px 안팎으로 아주 작게 그려지는 헤더에서는 filter의
              feGaussianBlur가 도형 전체를 흐릿하게 뭉개 보이게 만들어서(특히 iOS Safari에서
              두드러짐) 제거했습니다. 이 크기에서는 그림자 자체가 어차피 거의 안 보입니다. */}
...
const LogoIcon = styled.svg<{ $scale: number }>`
  width: ${(props) => Math.ceil(30.813 * props.$scale)}px;
  height: ${(props) => Math.ceil(24 * props.$scale)}px;
  flex-shrink: 0;
`;

// 워드마크 글자(특히 "통"의 받침 "ㅇ")가 viewBox 아래쪽 끝까지 거의 여백 없이 꽉 차 있어서,
// 이 컴포넌트가 계산한 소수점 픽셀 크기(예: 20.793px)를 브라우저가 반올림/스냅하는 과정에서
// 그 아래쪽 1px 안팎이 잘려 보이는 경우가 있었습니다. viewBox 원래 정수 치수(79x21)까지
// 올림해서 여백을 살짝 더 주고, 소수점 픽셀 크기 자체를 피합니다.
const LogoWordmark = styled.svg<{ $scale: number }>`
  width: ${(props) => Math.ceil(79 * props.$scale)}px;
  height: ${(props) => Math.ceil(21 * props.$scale)}px;
  flex-shrink: 0;
`;
